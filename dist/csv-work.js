...
const replaceAllBack = (allRowItems) => {
...
const splitRow = (csvRow) => {
    ...
    if (positionCommaSeperator == -1) {
        ...
        const originalSubStringWithCommas = csvRow.substring(csvRow.indexOf('"'), csvRow.lastIndexOf('"'));
        const subStringWithNoCommas = originalSubStringWithCommas.replace(/,/g, ";");
...
const exportCsv = (csvHeader, csvMatrix, numberOfTable, nameOfTable) => {
    const csvFromArrayOfArray = convertArrayToCSV(csvMatrix, {
        header: csvHeader,
        separator: ",",
    });
    if (numberOfTable != 0) {
        fs.writeFile(path.resolve(`assets/ohad-table ${numberOfTable}.csv`), csvFromArrayOfArray, (err) => {
            if (err) {
                console.log("error");
            }
            console.log("done");
    ...
    if (numberOfTable == 0) {
        fs.writeFile(path.resolve(`assets/ohad-table ${nameOfTable}.csv`), csvFromArrayOfArray, (err) => {
            if (err) {
                console.log("error");
            }
            console.log("done");
...
const paginateCsv = (csvToString, paginationNumber) => {
    ...
    for (let i = 0; i < csvMatrix.length; i += paginationNumber) {
        tableNum++;
        exportCsv(csvHeader, csvMatrixWithoutHeader.slice(i, i + paginationNumber), tableNum);
    }
};
const equalFilter = (csvMatrix, numberOfColumn, filteringNumber) => {
    const filteredArray = [];
...
const lessThanFilter = (csvMatrix, numberOfColumn, filteringNumber) => {
    const filteredArray = [];
    for (let i = 0; i < csvMatrix.length; i++) {
        if (parseFloat(csvMatrix[i][numberOfColumn]) < filteringNumber) {
            filteredArray.push(csvMatrix[i]);
...
const moreThanFilter = (csvMatrix, numberOfColumn, filteringNumber) => {
    const filteredArray = [];
    for (let i = 0; i < csvMatrix.length; i++) {
        if (parseFloat(csvMatrix[i][numberOfColumn]) > filteringNumber) {
            filteredArray.push(csvMatrix[i]);
...
const filterCsv = (numberOfColumn, filter, filteringVariable) => {
...
const switchRowsMatrix = (csvMatrix, numberOfRow) => {
    let saveRow = [];
...
const AscSort = (csvMatrix, numberOfColumn) => {
    ...
    for (let i = 1; i < csvMatrix.length; i++) {
        for (let j = 0; j < csvMatrix.length - 1; j++) {
            if (!Number.isNaN(parseFloat(checkTypeVar))) {
                if (parseFloat(csvMatrix[j][numberOfColumn]) >
...
const DescSort = (csvMatrix, numberOfColumn) => {
    const checkTypeVar = csvMatrix[1][numberOfColumn];
    for (let i = 1; i < csvMatrix.length; i++) {
        for (let j = 0; j < csvMatrix.length - 1; j++) {
            if (!Number.isNaN(parseFloat(checkTypeVar))) {
...
const sortCsv = (numberOfColumn, order) => {
    let exportArray = [];
    const ArrayCsv = stringToArray(csvToString);
    switch (order) {
        case "ASC":
...
runCode();
//# sourceMappingURL=csv-work.js.map